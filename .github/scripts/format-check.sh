...
#
# No diagnostics are excluded by default. Set FORMAT_CHECK_EXCLUDED_DIAGNOSTICS
# (space-separated rule IDs) if a specific rule needs excluding later (e.g. a
# source generator that MSBuildWorkspace can't resolve, causing false positives).
...
excluded_diagnostics="${FORMAT_CHECK_EXCLUDED_DIAGNOSTICS:-}"
...
format_args=(format "$solution" --verify-no-changes --severity info --no-restore)
if [[ -n "$excluded_diagnostics" ]]; then
  # shellcheck disable=SC2206 # excluded_diagnostics is an intentional word-split list
  format_args+=(--exclude-diagnostics $excluded_diagnostics)
fi

set +e
"$dotnet_bin" "${format_args[@]}" > "$output_file" 2>&1
format_exit=$?
set -e
